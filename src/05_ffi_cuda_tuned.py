"""Tuned CUDA kernel, called through `jax.ffi`."""
...
_TARGET = "qk_norm_rope_cuda_tuned"
...
def load_library():
  ...
  jax.ffi.register_ffi_target(
    _TARGET, jax.ffi.pycapsule(lib.QkNormRopeCudaTuned), platform="CUDA"
  )
  return lib


def cuda_qk_norm_rope_tuned(queries, gamma, cos, sin):
  """QK-Norm+RoPE."""
...
def main():
  ...
  err = kernel.check(jax.jit(cuda_qk_norm_rope_tuned))
  print(f"shape = {kernel.CHECK_SHAPE}  max abs error: {err:.3e}")

  ms = kernel.bench(cuda_qk_norm_rope_tuned)
